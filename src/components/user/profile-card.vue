<script lang="ts" setup>
import { computed } from "vue";
import { useRouter } from "vue-router";
import { useStore } from "vuex";

const store = useStore();
const user = computed(() => store.state.user);
const router = useRouter();
function showLogin() {
    if (user.value.data && user.value.isSuccess)
        return router.push({ name: "user-profile" });
    store.commit("toggleLoginPopup");
}
</script>

<template>
    <button class="header__profile" @click="showLogin()">
        <svg
            width="16"
            height="19"
            viewBox="0 0 16 19"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
        >
            <path
                d="M3 5C3 3.67392 3.52678 2.40215 4.46447 1.46447C5.40215 0.526784 6.67392 0 8 0C9.32608 0 10.5979 0.526784 11.5355 1.46447C12.4732 2.40215 13 3.67392 13 5C13 6.32608 12.4732 7.59785 11.5355 8.53553C10.5979 9.47322 9.32608 10 8 10C6.67392 10 5.40215 9.47322 4.46447 8.53553C3.52678 7.59785 3 6.32608 3 5ZM8 8C8.79565 8 9.55871 7.68393 10.1213 7.12132C10.6839 6.55871 11 5.79565 11 5C11 4.20435 10.6839 3.44129 10.1213 2.87868C9.55871 2.31607 8.79565 2 8 2C7.20435 2 6.44129 2.31607 5.87868 2.87868C5.31607 3.44129 5 4.20435 5 5C5 5.79565 5.31607 6.55871 5.87868 7.12132C6.44129 7.68393 7.20435 8 8 8Z"
                fill="white"
            />
            <path
                d="M2.343 13.343C1.60014 14.0859 1.01088 14.9678 0.608869 15.9385C0.206856 16.9091 -3.82755e-05 17.9494 5.31123e-09 19H2C2 17.4087 2.63214 15.8826 3.75736 14.7573C4.88258 13.6321 6.4087 13 8 13C9.5913 13 11.1174 13.6321 12.2426 14.7573C13.3679 15.8826 14 17.4087 14 19H16C16.0001 17.4177 15.5309 15.8709 14.6518 14.5553C13.7728 13.2396 12.5234 12.2142 11.0615 11.6087C9.59966 11.0032 7.99108 10.8447 6.4392 11.1534C4.88731 11.4621 3.46182 12.2241 2.343 13.343Z"
                fill="white"
            />
        </svg>
    </button>
</template>
<style lang="scss">
.header {
    &__profile {
        background: #252930;
        border-radius: 10px;
        width: 40px;
        height: 40px;
        display: flex;
        justify-content: center;
        align-items: center;
    }
    &__profile {
        background: #0d0d0d;
    }
}
</style>
