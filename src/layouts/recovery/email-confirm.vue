...
const isUserAccountActive = computed(() => store.state.user.data.is_verified);
if (isUserAccountActive.value) router.push("/");
const { doAsyncQuery, isLoading, isError } = useAsyncQuery(confirmEmail, () => {
    store.dispatch("getUser");
    router.push("/");
});

doAsyncQuery<string>(token);
</script>
<template>
